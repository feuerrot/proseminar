...
The Border Gateway Protocol as specified in \cite{RFC4271} can be used either as an IGP inside an AS or as an EGP between different AS, but we will study it as an EGP. BGP exchanges network layer reachability information (NLRI) between two peers by TCP and a protocol tracking finite state machine at each peer to allow for destination based forwarding. The received NLRI are filtered, saved to a local routing information base (RIB) which is later used to build the routing table and finally distributed to other peers. 
...
% von oben nach unten
    ...
    (I) edge [loop above] node {other} (I)
        edge [bend left=20] node[right=1mm] {1} (C)
        
    (C) edge [loop right] node {9} (C)
        edge [bend left=20] node[right=1mm] {?} (A)
        edge [bend left=50] node[right=1mm] {16, 17} (OS)
...
% von unten nach oben
    (E) edge [bend left=70]   node[right=1mm] {other} (I)
    (OC) edge [bend right=40] node[left=1mm] {other} (I)
    (OS) edge [bend left=60] node[right=1mm] {other} (I)
         edge [bend left=20]  node[left=1mm] {18} (A)
    (A) edge [bend left=20]   node[left=1mm] {9} (C)
        edge [bend left=50]   node[left=1mm] {other} (I)
    (C) edge [bend left=20]   node[left=1mm] {other} (I);
\end{tikzpicture}
\caption{The FSM of a BGP connection}
\label{fig:bgpfsm}
\end{figure}

Every BGP session consists of eight mandatory attributes including the current state of the finite state machine, a ConnectRetryCounter and -Time, a HoldTime, a KeepaliveTime and associated timer. Furthermore there are 16 mandatory state machine events from which at least four are used for a successfull setup of the peering. The FSM as seen in \ref{fig:bgpfsm} starts in the idle state, where no ressources are allocated and incoming connections are refused. On a ManualStart (1) event the system initialises the used ressources which are the ConnectRetryCounter and the ConnectRetryTimer. Then the system listens for incoming connections and initiates one to the peer system and is then in the connect state. If the ConnectRetryTimer expires, the system stays in this state and tries to reconnect to the peer system. Else if there is a TCP\_CR\_Acked (16) or TCPConnectionConfirmed (17) event, the ConnectRetryTimer stops and the BGP session setup progresses through sending an OPEN message containing amongst other data the version number 4, the number of the AS and a BGP identifier which must be an IP address assigned to the host. The state changes to OpenSend where the host waits for an OPEN message of its peer (Event 19: BGPOpen) which is answered by a KEEPALIVE message which does not have data except for the header. After setting the KeepaliveTimer and HoldTimer the state changes to OpenConfirm, where the host waits for the KEEPALIVE message of its peer (Event 26: KeepAliveMsg) which changes the state to Established upon reception.

The events ManualStop(2), ConnectRetryTimerExpires(9), HoldTimerExpires(10), KeepaliveTimerExpires(11), TcpConnectionFails(18), BGPHeaderError(21), BGPOpenMsgError(22), NotifMsgVerErr(24), NotifMsg(25), UpdateMsgErr(28) lead to the teardown of the current connection attempt through dropping of the TCP connection and resetting of used timers and ressources before the state changes to Idle again.

The Established state is the only state where UpdateMsg (27) events indicating the reception of an UPDATE message are permitted. An UPDATE message consists of the BGP packet header followed by the withdrawn routes, path attributes and NLRI for these path attributes. Withdrawn routes should be deleted from the local RIB (loc-RIB) at once. There are different types of path attributes which are mandatory and well-known, well-known and optional. Attributes which are mandatory must be included in UPDATE packets, well-known attributes must be understand by a implementation of BGP. There are three mandatory attributes: ORIGIN (Type 1) which specifies the origin of the path information, AS\_PATH (Type 2) which contains a set of every AS till the ORIGIN and NEXT\_HOP (Type 3) which specifies the unicast IP address of the next hop to reach the destination.
